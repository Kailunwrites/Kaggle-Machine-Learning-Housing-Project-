...
#imputing LotFrontage....? (it's already done)
# nbhd <- hp_orig %>% group_by(Neighborhood) %>% summarise(a = mean(LotFrontage)) %>% mutate(Neighborhood = as.numeric(Neighborhood))

# hp_orig %>% select(Neighborhood, LotFrontage) %>% 
#   mutate(LotFrontage = ifelse(is.na(LotFrontage),
#                           nbhd$a[as.numeric(Neighborhood)],
